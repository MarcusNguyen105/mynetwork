#!/usr/bin/env python3

ciphertext = """P^8(#R|X^gH#=XhR#C(|8^=R#RX|CHX^(}^|#^|8=HX(Cb]^Cb|C^>(R8-}^|^}R3hR#gR^Hw^m}Rh-HdX|#-H]^#h]BRX}^g|8gH8|CR-^S(Cb^}R.RX|8^]|CbR]|C(g|8^HmRX|C(H#}*
vbR^]RCbH-^XRmXR}R#C}^H#R^Hw^CbR^H8-R}C^|#-^BR}Cdr#HS#^m}Rh-HX|#-H]^#h]BRX^=R#RX|CHX^|8=HX(Cb]}*
vbR^CbRHX>^BRb(#-^CbR]^(}^XR8|C(.R8>^R|}>^CH^h#-RX}C|#-?^|#-^CbR>^|XR^R|}(8>^(]m8R]R#CR-^|#-^w|}C?^R}mRg(|88>^H#^gH]mhCRX^b|X-S|XR^Sb(gb^g|#^mXH.(-R^]H-h8|X^|X(Cb]RC(g^B>^}CHX|=RdB(C^CXh#g|C(H#*
k;p}^|XR^w|}C^|#-^XR3h(XR^](#(]|8^]R]HX>^iH#R^]H-h8Hd]^#h]BRX?^HwCR#^%_^HX^5J^B(C}/^CH^XRC|(#^}C|CR*
vb(}^]|rR}^CbR]^.|8h|B8R^wHX^}(]h8|C(#=^]h8C(m8R^(#-RmR#-R#C^}CXR|]}*
k;p}^|XR^#HC^(#CR#-R-?^|#-^]h}C^#HC^BR^h}R-?^wHX^gX>mCH=X|mb(g^|mm8(g|C(H#}0^h}R^|^gX>mCH=X|mb(g|88>^}RghXR^m}Rh-HX|#-H]^#h]BRX^=R#RX|CHX^wHX^}hgb^|mm8(g|C(H#}*
P8CbHh=b^k;p}^b|.R^|^wRS^}mRg(w(g^SR|r#R}}R}?^]|#>^Hw^CbR(X^w8|S}^gH]R^wXH]^b|.(#=^CHH^}]|88^|^}C|CR*^vbR^w|gC^Cb|C^mRHm8R^b|.R^BRR#^8h88R-^wHX^}H^]|#>^>R|X}^(#CH^h}(#=^CbR]^S(Cb^}hgb^}]|88^]H-h8(^g|#^BR^}RR#^|}^|^CR}C|]R#C^CH^}CXR#=Cb^Hw^CbR^CRgb#(3hR*^P^k;p^S(Cb^8|X=R^R#Hh=b^}C|CR^g|#^m|}}^R.R#^}CX(#=R#C^}C|C(}C(g|8^CR}C}0^|^]H-h8Hd_^k;p^Sb(gb^XRChX#}^CbR^b(=b^%_^B(C}^m|}}R}^vR}C<t:D}^Q]|88;Xh}b^}h(CR?^|#-^|^E5dB(C^k;p^Sb(gb^m|}}R}^CbR^]H}C^}CX(#=R#C^u(=;Xh}b^}h(CR*
vbR^w8|=^(}^<Q;;)g5BRR% :%:_ ZEo|gBoRoJogE g5oEJgs*"""

# Remove newlines for easier processing
cipher_clean = ciphertext.replace('\n', ' ')

def xor_bytes(b1, b2):
    return bytes([a ^ b for a, b in zip(b1, b2)])

# We know ^ is space, so key byte at those positions is ord('^') ^ ord(' ') = 94 ^ 32 = 126
print("Analyzing key structure...")
print("="*60)
print(f"'^' XOR ' ' = {ord('^') ^ ord(' ')}")

# Let's build the key by finding where spaces are
# and using known words

# Build key array
key = [None] * 100  # Start with a large array

# Known: "vbR^" -> "The "
for i, (p, c) in enumerate(zip("The ", "vbR^")):
    key[i] = ord(c) ^ ord(p)

print(f"Initial key from 'The ': {key[:10]}")

# Let's try to extend by guessing the plaintext
# Looking at "vbR^]RCbH-^XRmXR}R#C}" 
# This is likely "The method represents" or "The method represent"

guesses = [
    ("A linear congruential generator is an algorithm that yields a sequence of pseudo-random numbers calculated with several mathematical operations.", 
     cipher_clean),
]

for guess, cipher_text in guesses:
    if len(guess) <= len(cipher_text):
        # Extract all key bytes from this guess
        test_key = []
        for i, (p, c) in enumerate(zip(guess, cipher_text)):
            kb = ord(c) ^ ord(p)
            test_key.append(kb)
        
        print(f"\nExtracted {len(test_key)} key bytes")
        print(f"First 50 key bytes: {test_key[:50]}")
        
        # Look for repeating patterns
        print("\nLooking for key period...")
        for period in range(2, 50):
            # Check if key repeats with this period
            matches = 0
            total = 0
            for i in range(period, min(len(test_key), 200)):
                if test_key[i] == test_key[i % period]:
                    matches += 1
                total += 1
            
            if total > 0 and matches / total > 0.9:  # 90% match
                print(f"  Period {period}: {matches}/{total} = {100*matches/total:.1f}% match")
                if matches / total > 0.95:
                    print(f"  ** Likely key period: {period} **")
                    actual_key = test_key[:period]
                    print(f"  Key: {actual_key}")
                    
                    # Try to decrypt with this key
                    result = []
                    for i, c in enumerate(cipher_text):
                        result.append(chr(ord(c) ^ actual_key[i % period]))
                    
                    decrypted = ''.join(result)
                    print(f"\n{'='*60}")
                    print(f"Decryption with period {period}:")
                    print(f"{'='*60}")
                    print(decrypted[:600])
                    print("\n[Full decryption]")
                    print(decrypted)
                    break
