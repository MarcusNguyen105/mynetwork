...
def test_rot13():
    """Test ROT13 on the encrypted message"""
    print("Testing ROT13...")
    
    # Test on a small segment first
    test_segment = "vbR"
    decrypted_segment = rot13(test_segment)
    print(f"'vbR' -> '{decrypted_segment}'")
    
    if decrypted_segment.lower() == "the":
        print("ROT13 works! Decrypting full message...")
        
        decrypted_full = rot13(encrypted_message)
        print("\nDecrypted message:")
        print(decrypted_full)
        
        # Look for the flag
        if "USCC{" in decrypted_full:
            flag_start = decrypted_full.find("USCC{")
            flag_end = decrypted_full.find("}", flag_start)
            if flag_end != -1:
                flag = decrypted_full[flag_start:flag_end+1]
                print(f"\n🎉 FOUND FLAG: {flag}")
                return flag
        else:
            print("No flag found in decrypted text")
            
        return decrypted_full
    else:
        print("ROT13 doesn't seem to work")
        return None
...
if __name__ == "__main__":
    ...
    if not result:
        # Try other common rotations
        print("\nTrying other rotations...")
        for rot in [1, 3, 5, 7, 11, 17, 19, 23, 25]:
            test_segment = "vbR"
            decrypted = ""
            for char in test_segment:
                if char.isalpha():
                    base = ord('A') if char.isupper() else ord('a')
                    decrypted += chr((ord(char) - base + rot) % 26 + base)
                else:
                    decrypted += char
            
            print(f"ROT{rot}: 'vbR' -> '{decrypted}'")
            if decrypted.lower() == "the":
                print(f"ROT{rot} works!")
                break