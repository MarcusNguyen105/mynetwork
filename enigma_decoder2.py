#!/usr/bin/env python3
"""
Enigma decoder - trying different approaches
"""

from enigma.machine import EnigmaMachine

# The encrypted message (without spaces and header)
...
# Check if "WHF" from Kenngruppen day 20 is related to the indicator
print("Day 20 has 'WHF' in Kenngruppen: YGC WHF YDK UML")
print("Let's try using WHF and related positions:")
print()

for init_pos in ["WHF", "YGC", "YDK", "UML", "GZV", "DBR"]:
    result = try_decode(20, init_pos)
    print(f"Day 20, Position {init_pos}:")
    print(f"  {result}")
    print()

# Also try the full message with all possible 3-letter combinations from indicator
print("\n" + "="*80)
print("Trying GZV, DBR, and combinations:")
print("="*80)

for day in range(1, 31):
    for init_pos in ["GZV", "DBR", "ZVD", "VDB"]:
        result = try_decode(day, init_pos)
        # Check for common English words or patterns
        if any(word in result.upper() for word in ["CONGRATULATIONS", "FLAG", "WELLDONE", "BLETCHLEY", "ENIGMA", "SUCCESS"]):
            print(f"\n*** POSSIBLE MATCH ***")
            print(f"Day {day}, Position {init_pos}:")
            print(f"  {result}")
            print()
