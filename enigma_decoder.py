#!/usr/bin/env python3
"""
Enigma decoder for the mysterious message
"""

from enigma.machine import EnigmaMachine

# The encrypted message (without spaces)
encrypted_message = "XLWHFDJKUCZONWZUFDGBSIILKGSOBRNPPMFBWFTUCHPCOUUBMBNUUMWHMPJGJGJPMAXKPYFENEPLKHGMLUPUJWWCZOYATTSCBSKIQFKSGADRPZJ"

# Codebook settings for each day
# Format: day: (rotors, ring_settings, plugboard, kenngruppen)
codebook = {
...
def format_plugboard(plugboard_str):
    """Convert plugboard string to the format required by py-enigma"""
    return plugboard_str  # Keep the spaces between pairs

def try_decode(day, initial_position="AAA"):
    """Try to decode the message using settings for a specific day"""
    rotors, ring_settings, plugboard, kenngruppen = codebook[day]
    
    # Convert rotor names to the format used by py-enigma
    ...
    machine = EnigmaMachine.from_key_sheet(
        ...
        reflector='B',  # Standard reflector B for Wehrmacht
        ring_settings=' '.join([str(r) for r in ring_list]),
        plugboard_settings=format_plugboard(plugboard)
    )
    ...
    # Decrypt the message
    plaintext = machine.process_text(encrypted_message)
    
    return plaintext

# The hint "GZV DBR" might be the indicator
# Let's also try decoding the indicator first with different days
indicator = "GZVDBR"

print("Trying to decode the indicator 'GZV DBR' with different day settings:")
print("=" * 80)

for day in range(1, 31):
    # Try with initial position AAA to decode the indicator
    plaintext_indicator = try_decode(day, "AAA")[:6]
    print(f"Day {day:2d}: Indicator decoded as: {plaintext_indicator}")
    
    # Now try to decode the main message with that as the initial position
    if len(plaintext_indicator) >= 3:
        message_key = plaintext_indicator[:3]
        full_message = try_decode(day, message_key)
        
        # Check if the output looks like German text
        if any(word in full_message.upper() for word in ["THE", "AND", "FLAG", "CONGRATULATIONS", "WELL", "DONE", "YOU", "HAVE", "USCC"]):
            print(f"\n{'*' * 80}")
            print(f"POSSIBLE SOLUTION FOUND FOR DAY {day}!")
            print(f"Message key: {message_key}")
            print(f"Decoded message: {full_message}")
            print(f"{'*' * 80}\n")

print("\n" + "=" * 80)
print("Also trying direct decryption with various initial positions:")
print("=" * 80)

# Try some common initial positions
for day in [4, 9, 12, 20]:  # Focus on days that might be relevant
    print(f"\nDay {day}:")
    for init_pos in ["AAA", "GZV", "DBR"]:
        result = try_decode(day, init_pos)
        print(f"  Position {init_pos}: {result[:50]}...")
