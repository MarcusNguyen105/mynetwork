#!/usr/bin/env python3
"""
Final comprehensive brute force
"""
...
# Generate all reasonable 3-letter positions
test_positions = ["GZV", "DBR", "AAA", "WHF"]

# Also generate some positions from the alphabet
for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
    test_positions.append(letter * 3)  # AAA, BBB, CCC, etc.

print(f"Testing {len(test_positions)} positions across 30 days...")
...
for day in range(1, 31):
    ...
    for pos in test_positions[:20]:  # Limit to first 20 positions for speed
        for method in ["direct", "indicator"]:
            try:
                if method == "direct":
                    ...
                    machine1.set_display(pos)
                    ind = machine1.process_text(full_encrypted[:6])
                    ...
                    machine2.set_display(ind[:3])
                    text = machine2.process_text(full_encrypted[6:])
                
                total_tests += 1
                
                # Check for actual English words  
                if "USCC{" in text or "FLAG" in text or "CONGRATULATIONS" in text:
                    matches.append((day, pos, method, text))
                    print(f"\n*** POTENTIAL MATCH ***")
                    print(f"Day {day}, Position {pos}, Method: {method}")
                    print(f"Text: {text}")
...
if matches:
    ...
    for day, pos, method, text in matches:
        print(f"\nDay {day}, Pos={pos}, Method={method}:")
        print(f"  {text}")
else:
    print("No clear matches found. The challenge may require:")
    print("  - A different Enigma model (M4 with 4 rotors?)")
    print("  - Different reflector or settings interpretation")
    print("  - Additional cipher layer after Enigma decryption")
