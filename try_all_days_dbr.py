#!/usr/bin/env python3
"""
Try all days with DBR ground setting and first 6 letters as indicator
"""
...
print("Trying all days with DBR ground setting, decrypting first 6 letters as indicator:")
print("="*80)

keywords = ["CONGRATULATIONS", "WELLDONE", "FLAG", "WELL", "DONE", "SUCCESS", "CRACK", "DECIPHER", "THE", "AND", "THIS", "THAT"]

for day in range(1, 31):
    rotors, ring_settings, plugboard = codebook[day]
    
    # Decrypt indicator with DBR
    ...
    plaintext = machine2.process_text(full_encrypted[6:])
    
    # Check for keywords
    found = [w for w in keywords if w in plaintext.upper()]
    
    if found:
        print(f"\nDay {day}: Indicator={indicator}, Key={msg_key}")
        print(f"  Plaintext: {plaintext}")
        print(f"  Found keywords: {found}")
    
    # Also check for USCC specifically
    if "USCC" in plaintext:
        print(f"\n*** FOUND USCC IN DAY {day} ***")
